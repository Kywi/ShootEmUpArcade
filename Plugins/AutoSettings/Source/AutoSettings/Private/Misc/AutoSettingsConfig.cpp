...
void FInputMappingGroup::ReplaceAction(FInputActionKeyMapping Action, bool bAnyKeyGroup)
{
	...
	FGameplayTag KeyGroup = bAnyKeyGroup ? FGameplayTag() : GetDefault<UAutoSettingsConfig>()->GetKeyGroup(Action.Key);
...
{
	bool bIsAnalog = GetDefault<UAutoSettingsConfig>()->IsAnalogKey(Axis.Key);

	// Filter key group unless using any
	FGameplayTag KeyGroup = bAnyKeyGroup ? FGameplayTag() : GetDefault<UAutoSettingsConfig>()->GetKeyGroup(Axis.Key);
...
void FInputMappingPreset::SetMappings(TArray<FInputActionKeyMapping> ActionMappings, TArray<FInputAxisKeyMapping> AxisMappings)
{
	...
	for (FInputAxisKeyMapping Axis : AxisMappings)
	{
		...
		bool IsAnalog = GetDefault<UAutoSettingsConfig>()->IsAnalogKey(Axis.Key);
...
void FInputMappingPreset::Apply(APlayerController* Player)
{
	...
	// then add ones from GetAction() onto it (thus preserving any actions from the backlist)
