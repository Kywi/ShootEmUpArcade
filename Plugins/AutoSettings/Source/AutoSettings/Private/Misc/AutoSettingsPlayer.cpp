...
FString IAutoSettingsPlayer::GetUniquePlayerIdentifier(const APlayerController* PlayerController)
{
	...
	return PlayerController->Implements<UAutoSettingsPlayer>() ? IAutoSettingsPlayer::Execute_GetUniquePlayerIdentifier(PlayerController) : GetUniquePlayerIdentifier_Default(PlayerController);
}
...
{
	check(PlayerController);
	return PlayerController->Implements<UAutoSettingsPlayer>() ? IAutoSettingsPlayer::Execute_GetDefaultInputMappingPreset(PlayerController) : GetDefaultInputMappingPreset_Default();
}
...
{
	check(PlayerController);
	return PlayerController->Implements<UAutoSettingsPlayer>() ? IAutoSettingsPlayer::Execute_GetInputMappings(PlayerController, InputMappings) : false;
}
...
void IAutoSettingsPlayer::SaveInputMappings(APlayerController* PlayerController, FPlayerInputMappings InputMappings)
{
	...
	{
		IAutoSettingsPlayer::Execute_SaveInputMappings(PlayerController, InputMappings);
	}
}
