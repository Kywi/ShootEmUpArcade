...
void UAutoSettingWidget::Save()
{
	...
	FString FinalConfigValue = ProcessOutgoingValue(CurrentValue, true);
...
void UAutoSettingWidget::Cancel()
{
	...
	FString ConfigValue = ProcessIncomingValue(USettingsManager::GetValue(CVarName, true));
...
void UAutoSettingWidget::ConsoleVariableSink()
{
	...
	FString CVarValue = ProcessIncomingValue(UConsoleUtils::GetStringCVar(CVarName));
...
void UAutoSettingWidget::ApplySettingValue(FString Value, bool bSaveIfPossible)
{
	...
	bool bShouldSave = bAutoSave && bSaveIfPossible;
...
void UAutoSettingWidget::ApplyInternal(bool bPendingSave)
{
	...
	FString FinalCVarValue = ProcessOutgoingValue(CurrentValue, false);
...
FString UAutoSettingWidget::ProcessIncomingValue(FString Value)
{
	if (ValueMask)
	{
...
FString UAutoSettingWidget::ProcessOutgoingValue(FString Value, bool bRecombineWithConfigValue)
{
	if (ValueMask)
	{
		FString RetrievedValue = bRecombineWithConfigValue ? USettingsManager::Get()->GetConfigValue(CVarName) : UConsoleUtils::GetStringCVar(CVarName);
