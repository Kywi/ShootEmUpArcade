...
void ANWPlayerPawn::BeginPlay()
{
	...
	interpToLocation = GetActorLocation();
...
void ANWPlayerPawn::PossessedBy(AController* NewController)
{
	...
	healtComponent->HealthsChanged.AddDynamic(this, &ANWPlayerPawn::OnHealthChanged);
	InitTouchLocation();
}

void ANWPlayerPawn::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);

	SetActorLocation(currentLocation);
	currentLocation = FMath::VInterpTo(currentLocation, interpToLocation, DeltaSeconds, interpSpeedSmoothMoving);
...
void ANWPlayerPawn::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ANWPlayerPawn, interpToLocation);
}
...
void ANWPlayerPawn::OnTouchMove(ETouchIndex::Type FingerIndex, FVector Location)
{
	...
	if (IsValid(InputComponent))
	{
		...
		SetInterpToLocation(NewLocation);
		//SetActorLocation(NewLocation, true);
		//if (GetNetMode() == ENetMode::NM_ListenServer)
		//	SmoothMovingPawn(NewLocation);
...
#pragma endregion Getting damage
